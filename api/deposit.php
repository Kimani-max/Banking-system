    ...
    if ($accountId && $amount > 0) {
        try {
            ...
            $stmt = $pdo->prepare("
                ...
                VALUES 
                (:acct, :cust, 'deposit', :amt, 'Cash deposit', NOW(), :bal)
            ");
            $stmt->execute([
                ':acct' => $accountId,
                ':cust' => $customerId,
...
<div class="teller-form-container">
  <h2 class="teller-form-title">💰 Deposit Money</h2>
  ...
  <form method="post" class="teller-form">
    ...
    <label for="amount">Deposit Amount (Ksh):</label>
    <input type="number" step="0.01" name="amount" required>

    <button type="submit" class="teller-btn">Deposit Funds</button>
  </form>
</div>
</body>
</html>
